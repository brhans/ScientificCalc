package com.example.tamsi.scientificcalculator;
...
public class Calculator {
        ...
        evaluator = new DoubleEvaluator(params) {
            ...
            protected Double evaluate(Function function, Iterator arguments, Object evaluationContext) {
                if (function == sqrt)
                    ...
                    double ans = Math.sin( Math.toRadians((double)arguments.next() ) );
                    ans = Math.round(ans*100.0) / 100.0;
                    return ans;
                }
                else if (function == cos) {
                    double ans = Math.cos( Math.toRadians((double)arguments.next() ) );
                    ans = Math.round(ans*100.0) / 100.0;
                    return ans;
                }
                else if (function == tan) {
                    double ans = Math.tan( Math.toRadians((double)arguments.next() ) );
                    ans = Math.round(ans*100.0) / 100.0;
                    return ans;
                }
    ...
    public String getResult(String currentDisplay, String expressionUsedForParsing, boolean isDegree) {
...
//            Log.e("radian", String.valueOf(sum) + " " + newSum);
//        } else {
//            newSum = Math.toRadians(sum);
//            newSum = newSum * (180 / Math.PI);
//            Log.e("degree", String.valueOf(sum) + " " + newSum);
//        }
        return newSum;
    }
